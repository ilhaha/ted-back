...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public LoginResp examLogin(LoginReq req, HttpServletRequest request) {
        AuthTypeEnum authType = req.getAuthType();
        // 校验终端
        ClientResp client = clientService.getByClientId(req.getClientId());
        ValidationUtils.throwIfNull(client, "终端不存在");
        ...
        LoginHandler<LoginReq> loginHandler = loginHandlerFactory.getHandler(authType);
        // 登录
        LoginResp loginResp = loginHandler.examLogin(req, client, request);
        // 登录后置处理
        loginHandler.postLogin(req, client, request);
        return loginResp;
    }
    ...
    @Override
    public boolean forgotPassword(UserReq req) {
        String rawPhone = ExceptionUtils.exToNull(() -> SecureUtils.decryptByRsaPrivateKey(req.getPhone()));
        String rawPassword = ExceptionUtils.exToNull(() -> SecureUtils.decryptByRsaPrivateKey(req.getPassword()));
        return userService.forgotPassword(rawPhone, rawPassword);
    }

    @Override
    public LoginResp invigilatorLogin(LoginReq req, HttpServletRequest request) {
        AuthTypeEnum authType = req.getAuthType();
        // 校验终端
        ClientResp client = clientService.getByClientId(req.getClientId());
        ValidationUtils.throwIfNull(client, "终端不存在");
        ...
        LoginHandler<LoginReq> loginHandler = loginHandlerFactory.getHandler(authType);
        // 登录
        LoginResp loginResp = loginHandler.invigilatorLogin(req, client, request);
        // 登录后置处理
        loginHandler.postLogin(req, client, request);
        return loginResp;
    }
    ...
    public LoginResp candidatesOrInviteLogin(LoginReq req, HttpServletRequest request) {
        ...
        loginHandler.preLogin(req, client, request);
        // 登录
        LoginResp loginResp = loginHandler.candidatesOrInviteLogin(req, client, request);
        // 登录后置处理
        loginHandler.postLogin(req, client, request);
        return loginResp;
    }
