...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, UserDO, UserResp, UserDetailResp, UserQuery, UserReq> implements UserService, CommonUserService {
    ...
    public void exportExamStaffFee(Long userId, String exportDate, HttpServletResponse response) {
        ...
        if (totalAllFee.compareTo(threshold) > 0) {
            tax = totalAllFee.subtract(threshold).multiply(taxRate).setScale(2, RoundingMode.HALF_UP);
        }
    ...
    public ResponseEntity<byte[]> exportExamStaffFeeBatch(String exportDate) {
        ...
        try (Workbook workbook = new XSSFWorkbook(new ByteArrayInputStream(templateBytes));
            ...
            for (Map.Entry<Long, List<ExamStaffFeeExportVO>> entry : grouped.entrySet()) {
                ...
                //  拼接监考内容，多条换行
                String examContent = records.stream().map(r -> String.format("%s: %s %s %s %s", r.getStartTime(),       // 考试日期
                ...
                summaryList.add(new ExamStaffFeeSummaryVO(entry.getKey(), examStaffFeeExportVO
                    .getNickname(), totalFee, taxFee, actualFee, examContent, examStaffFeeExportVO
                        .getInvigilatorIdNumber(), examStaffFeeExportVO.getInvigilatorBankName(), examStaffFeeExportVO
                            .getInvigilatorBankAccount(), examStaffFeeExportVO
                                .getInvigilatorUnit(), examStaffFeeExportVO.getInvigilatorTitle(), examStaffFeeExportVO
                                    .getPhone()));
            }
            ...
            if (summaryList.size() <= templateDataRowCount) {
                ...
                if (footerRow == null)
                    footerRow = sheet.createRow(fillDateIndex);
                ...
                if (footerRow == null)
                    footerRow = sheet.createRow(lastRowIndex);
                for (int col : targetCols) {
                    setCell(footerRow, col, currentDate);
                }
            }
            ...
            // 8. 重命名Sheet
            workbook.setSheetName(workbook.getSheetIndex(sheet), "劳动费" + formatted + "(签名)");
            ...
            headers.setContentDisposition(ContentDisposition.attachment()
                .filename("劳动费支出表（检验检测人员）" + exportDate + ".xlsx", StandardCharsets.UTF_8)
                .build());
     ...
     * 扩展数据区（超过34行自动在表尾前插入）
     */
    private void expandDataRowsIfNeeded(Sheet sheet,
                                        int dataSize,
                                        int dataStartRowIndex,
                                        int templateDataRowCount,
        ...
        //  整体下移表尾
        sheet.shiftRows(footerStartRowIndex, sheet.getLastRowNum(), extraRows, true, false);
