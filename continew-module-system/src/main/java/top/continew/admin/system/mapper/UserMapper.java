...
public interface UserMapper extends DataPermissionMapper<UserDO> {
    ...
    @Select("SELECT " + "ted_enroll.theory_score_reused, " + "ted_enroll.user_id AS candidate_id, " + "ted_exam_plan.id AS plan_id, " + "ted_exam_plan.start_time, " + "ted_exam_plan.end_time, " + "ted_exam_plan.exam_plan_name AS plan_name, " + "ted_exam_plan.status AS status, " + "ted_enroll.enroll_status AS enrollStatus, " + "ted_enroll.exam_status AS examStatus, " + "ted_enroll.classroom_id AS classroomId, " + "ted_project.video_url AS warningShortFilm, ted_category.enable_proctor_warning, " + "ted_project.exam_duration, ted_worker_apply.face_photo " + "FROM " + "ted_enroll " + "LEFT JOIN sys_user ON ted_enroll.user_id = sys_user.id " + "LEFT JOIN ted_worker_apply ON ted_enroll.class_id = ted_worker_apply.class_id AND sys_user.username = ted_worker_apply.id_card_number AND ted_worker_apply.is_deleted = 0 " + "LEFT JOIN ted_exam_plan ON ted_enroll.exam_plan_id = ted_exam_plan.id AND ted_exam_plan.is_deleted = 0 " + "LEFT JOIN ted_project ON ted_exam_plan.exam_project_id = ted_project.id AND ted_project.is_deleted = 0 " + "LEFT JOIN ted_category ON ted_project.category_id = ted_category.id AND ted_category.is_deleted = 0 " + "WHERE " + "ted_enroll.user_id = #{candidateId} " + "AND ted_enroll.exam_number = #{examNumber} " + " AND ted_enroll.is_deleted = 0")
