...
public class OrgServiceImpl extends BaseServiceImpl<OrgMapper, OrgDO, OrgResp, OrgDetailResp, OrgQuery, OrgReq> implements OrgService {
    ...
    public Boolean apply(OrgApplyReq orgApplyPreReq) {
        ...
        if (ObjectUtil.isNotEmpty(candidateTypeDOS)) {
            ...
            // candidateTypeId 列表
            List<Long> candidateTypeIds = candidateTypeDOS.stream()
                    .map(CandidateTypeDO::getId)
                    .toList();

            // 查询禁考项目
            List<CandidateTypeDisableProjectDO> disableProjects =
                    candidateTypeDisableProjectMapper.selectList(
                            new LambdaQueryWrapper<CandidateTypeDisableProjectDO>()
                                    ...
                                    .eq(CandidateTypeDisableProjectDO::getDisableProjectId, examProjectId)
                    );
            if (ObjectUtil.isNotEmpty(disableProjects)) {
                ...
                // 找到对应的 candidateId
                List<Long> forbiddenCandidateIds = candidateTypeDOS.stream()
                        .filter(ct -> forbiddenTypeIds.contains(ct.getId()))
                        .map(CandidateTypeDO::getCandidateId)
                        .toList();

                String blacklistNames = forbiddenCandidateIds.stream()
                        .map(id -> userMap.getOrDefault(id, "未知考生"))
                        .distinct()
                        .collect(Collectors.joining("、"));

                ValidationUtils.throwIf(Boolean.TRUE,
                        "以下考生已被加入黑名单，且禁止报考当前项目：" + blacklistNames);
            }
        }
    ...
    public ExcelParseResultVO parsedWorkerExcel(MultipartFile file, Long classId) {
        ...
        try (InputStream is = file.getInputStream(); XSSFWorkbook workbook = new XSSFWorkbook(is)) {
            ...
            // 一个班级最多50人，多出来的删掉
            // 班级现有人数
            Long classCurrentCount = workerApplyMapper.selectCount(
                    new LambdaQueryWrapper<WorkerApplyDO>()
                            .eq(WorkerApplyDO::getClassId, classId)
            );

            // 还可以导入多少人
            int remainQuota = 50 - classCurrentCount.intValue();

            // 如果已经满了
            if (remainQuota <= 0) {
                moveAllToFailed(successList, failedList, "该班级人数已满（50人）");
            } else {
                // 只允许导入剩余名额
                limitSuccessSize(successList, failedList, remainQuota);
            }
    ...
    private void moveAllToFailed(List<ExcelRowSuccessVO> successList,
                                 List<ExcelRowErrorVO> failedList,
                                 String message) {

        List<ExcelRowErrorVO> errorList = successList.stream()
                .map(item -> {
                    ExcelRowErrorVO errorVO = new ExcelRowErrorVO();
                    errorVO.setRowNum(item.getRowNum());
                    errorVO.setExcelName(item.getExcelName());
                    errorVO.setPhone(item.getPhone());
    ...
    private void limitSuccessSize(List<ExcelRowSuccessVO> successList,
                                  List<ExcelRowErrorVO> failedList,
                                  int maxSize) {

        if (successList.size() > maxSize) {

            // 复制超出部分（避免 subList 视图问题）
            List<ExcelRowSuccessVO> overflowList =
                    new ArrayList<>(successList.subList(maxSize, successList.size()));

            // 转失败记录
            List<ExcelRowErrorVO> overflowErrorList = overflowList.stream()
                    .map(item -> {
                        ExcelRowErrorVO errorVO = new ExcelRowErrorVO();
                        errorVO.setRowNum(item.getRowNum());
                        errorVO.setExcelName(item.getExcelName());
                        errorVO.setPhone(item.getPhone());
                        errorVO.setErrorMessage("超出班级人数上限（50人），超出部分已自动过滤");
                        return errorVO;
                    })
                    .toList();

            // 加入失败列表
            failedList.addAll(overflowErrorList);

            // 删除成功列表中超出部分
            successList.subList(maxSize, successList.size()).clear();
        }
    }
