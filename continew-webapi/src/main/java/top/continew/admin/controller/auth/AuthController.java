...
public class AuthController {
    ...
    @SaIgnore
    @Operation(summary = "考生考试端登录", description = "身份证登录")
    @PostMapping("/exam/login")
    public LoginResp examLogin(@Validated @RequestBody LoginReq req, HttpServletRequest request) {
        return authService.examLogin(req, request);
    }

    @SaIgnore
    @Operation(summary = "登录", description = "用户登录")
    @PostMapping("/candidatesOrInvite/login")
    public LoginResp candidatesOrInviteLogin(@Validated @RequestBody LoginReq req, HttpServletRequest request) {
        return authService.candidatesOrInviteLogin(req, request);
    }
    ...
    public UserInfoResp getUserInfo() {
        ...
        userInfoResp.setUsername(aesWithHMAC.verifyAndDecrypt(userInfoResp.getUsername()));
    ...
    public boolean forgotPassword(@RequestBody UserReq req) {
        return authService.forgotPassword(req);
    }

    @GetMapping("/user/findIsPhone")
    public boolean findIsPhone(@RequestParam("phone") String phone) {
        String rawPhone = ExceptionUtils.exToNull(() -> SecureUtils.decryptByRsaPrivateKey(phone));
        UserDO byPhone = userService.getByPhone(rawPhone);
        return byPhone != null;
    }

    @GetMapping("/candidatesOrInvite/findIsAccount")
    public boolean findIsAccount(String username) {
        return userService.findIsAccount(username);
    }
