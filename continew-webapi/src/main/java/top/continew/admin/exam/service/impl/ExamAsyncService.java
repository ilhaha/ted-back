...
public class ExamAsyncService {
    ...
    public void generateAdmissionTicket(ExamPlanDO planDO) {
        ...
        try {
            ...
            log.info("【准考证生成完成】考试计划 {}，成功处理 {} 条报名记录",
                    planId, ticketSaveList.size());

        } catch (Exception e) {
            log.error("【准考证异步任务异常】计划 {}，错误：{}",
    ...
    private String safeGenerateWorkerTicket(Long userId, String idCard, String encryptedExamNo, Long classId) {
        try {
            ...
            log.error("【PDF生成失败】用户ID={} 身份证={} 考试号={}，失败原因：{}",
                    userId, idCard, encryptedExamNo, e.getMessage());
            return null;
        }
    }
