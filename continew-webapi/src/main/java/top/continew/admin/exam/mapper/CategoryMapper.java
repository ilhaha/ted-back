...
public interface CategoryMapper extends BaseMapper<CategoryDO> {
     ...
     */
    @Select("<script>" + "SELECT `name` AS label, `id` AS `value` " + "FROM ted_category  " + "WHERE is_deleted = 0 " +
        // 动态拼接IN条件：有参数时过滤，无参数时查全部
        "<if test='categoryTypeList != null and categoryTypeList.size() > 0'>" + "  AND category_type IN " + "  <foreach collection='categoryTypeList' item='type' open='(' separator=',' close=')'>" + "    #{type}" + "  </foreach>" + "</if>" + "</script>")
