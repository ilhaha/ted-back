...
public class WeldingExamApplicationServiceImpl extends BaseServiceImpl<WeldingExamApplicationMapper, WeldingExamApplicationDO, WeldingExamApplicationResp, WeldingExamApplicationDetailResp, WeldingExamApplicationQuery, WeldingExamApplicationReq> implements WeldingExamApplicationService {
    ...
    public void update(WeldingExamApplicationReq req, Long id) {
        ...
        ValidationUtils.throwIf(count > 0, "项目【" + req.getProjectCode() + "】已存在");
        // 判断项目代码是否属于分焊接类型的
        ...
        //            );
        //        }
        baseMapper.update(new LambdaUpdateWrapper<WeldingExamApplicationDO>().eq(WeldingExamApplicationDO::getId, id)
    ...
    public PageResp<WeldingExamApplicationResp> page(WeldingExamApplicationQuery query, PageQuery pageQuery) {
        ...
        IPage<WeldingExamApplicationDetailResp> page = baseMapper.orgAndAdminPage(new Page<>(pageQuery
    ...
    public Boolean review(ReviewWeldingExamApplicationReq req) {
        ...
        ValidationUtils.throwIf(weldingExamApplicationDOS.stream()
            .anyMatch(item -> !WeldingExamApplicationStatusEnum.WAIT_REVIEW.getValue()
                .equals(item.getStatus())), "选中的记录中包含非未审核状态的数据，请重新选择");
        ...
        // 7. 更新审核状态
        List<WeldingExamApplicationDO> updateList = weldingExamApplicationDOS.stream().map(item -> {
    ...
    private void updateOrgCategoryRelations(Set<Long> orgIds, Long categoryId) {
        ...
        // 去掉已存在的
        Set<Long> toInsert = orgIds.stream().filter(id -> !existingOrgIds.contains(id)).collect(Collectors.toSet());

        if (toInsert.isEmpty())
            return;

        // 批量新增
        List<OrgCategoryRelationDO> newRelations = toInsert.stream().map(orgId -> {
