...
public class ExamPlanServiceImpl extends BaseServiceImpl<ExamPlanMapper, ExamPlanDO, ExamPlanResp, ExamPlanDetailResp, ExamPlanQuery, ExamPlanReq> implements ExamPlanService {
    ...
    public Boolean endExam(Long planId) {
        ...
        if (!notSignedInIds.isEmpty()) {
            ...
            enrollMapper.update(new LambdaUpdateWrapper<EnrollDO>()
                    .in(EnrollDO::getId, notSignedInIds)
            ...
            List<ExamRecordsDO> examRecordsList = signedUpList.stream().map(item -> {
                ...
                examRecordsDO.setOperInputStatus(hasOper ? ExamScoreEntryStatusEnum.NO_ENTRY.getValue() : ExamScoreEntryStatusEnum.ENTERED.getValue());
                examRecordsDO.setRoadScores(0);
                examRecordsDO.setRoadInputStatus(hasRoad ? ExamScoreEntryStatusEnum.NO_ENTRY.getValue() : ExamScoreEntryStatusEnum.ENTERED.getValue());
    ...
    private void insertBatchWeldingOperScore(List<ExamRecordsDO> examRecordsList) {
        ...
        Map<Long, String> recordWeldingMap = resultList.stream()
                .collect(Collectors.toMap(
                        map -> ((Number) map.get("record_id")).longValue(),
                        map -> (String) map.get("welding_project_codes")
                ));
    ...
    public Boolean centerDirectorConform(Long planId, Integer isFinalConfirmed) {
        ...
        if (success && isConfirmed && ExamPlanTypeEnum.WORKER.getValue().equals(examPlanDO.getPlanType())) {
            ...
            Map<Long, String> classNameMap = orgClassMapper.selectByIds(classIds)
                    .stream().collect(Collectors.toMap(OrgClassDO::getId, c -> Optional.ofNullable(c.getClassName()).orElse("")));
            ...
            for (EnrollDO enroll : enrollUpdateList) {
                ...
                // 生成准考证 PDF
                String ticketUrl = safeGenerateWorkerTicket(
                        user.getId(), user.getUsername(), user.getNickname(),
                        enroll.getExamNumber(),
                        enroll.getClassId(), classNameMap.get(enroll.getClassId())
                );
    ...
    public ExamCandidateInfoVO startExam(ExamPlanStartReq req) {
        ...
        boolean statusIsSuccess = PlanConstant.SUCCESS
                .getStatus().equals(examPlanDO.getStatus());

        ValidationUtils.throwIf(statusIsSuccess && !req.getExamPassword().equals(planInvigilateDO.getExamPassword()), "开考密码错误，请核对后重试");
