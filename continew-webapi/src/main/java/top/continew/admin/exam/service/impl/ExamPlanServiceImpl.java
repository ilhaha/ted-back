...
public class ExamPlanServiceImpl extends BaseServiceImpl<ExamPlanMapper, ExamPlanDO, ExamPlanResp, ExamPlanDetailResp, ExamPlanQuery, ExamPlanReq> implements ExamPlanService {
    ...
    public void importExcel(MultipartFile file) {
        ...
        try (InputStream is = file.getInputStream()) {
            ...
            for (int i = headerRowIndex + 2; i <= sheet.getLastRowNum(); i++) {
                ...
                // 解析考试时间
                LocalDateTime examStartTime = parseDate(examStart, "考试日期", rowIndex, formatter).toLocalDate()
                        .atTime(9, 0);
    ...
    public void inspectorUpdate(ExamPlanReq req, Long id) {
        ...
        // 校验监考员人数是否足够
        ValidationUtils.throwIf(invigilatorCount < totalClassrooms, "所设置的监考员人数不足以分配到全部考场");
    ...
    public void update(ExamPlanReq req, Long id) {
        ...
        if (HasExamEnum.YES.getValue().equals(projectDO.getIsTheory()))
            questionBankService.generateExamQuestionBank(id);
    ...
    public Boolean centerDirectorConform(Long planId, Integer isFinalConfirmed) {
        ...
        // 1. 校验 & 查询
        ExamPlanDO plan = getAndValidatePlan(planId);
        ProjectDO project = getProject(plan);

        boolean isConfirmed = PlanFinalConfirmedStatus.DIRECTOR_CONFIRMED.getValue().equals(isFinalConfirmed);

        // 2. 获取考场（统一）
        boolean isTheory = HasExamEnum.NO.getValue().equals(project.getIsTheory());
        List<Long> classroomIds = getClassroomIds(plan, isTheory);

        // 3. 查询报名
        List<EnrollDO> enrollList = getEnrollList(planId, isConfirmed, plan.getExamPlanName());

        // 4. 保存计划班级关系
        savePlanApplyClass(planId, enrollList);

        // 5. 监考员
        List<InvigilatorAssignResp> invigilatorList = getInvigilators(planId, plan.getExamPlanName());

        // 6. 驳回直接返回
        if (isRejected(isFinalConfirmed)) {
            return handleReject(planId, isFinalConfirmed);
        }

        // 7. 更新状态
        boolean success = updatePlanStatus(planId, isFinalConfirmed, isTheory);
    ...
    private ExamPlanDO getAndValidatePlan(Long planId) {
        ...
        ValidationUtils.throwIfNull(plan, "考试计划不存在");

        ValidationUtils.throwIf(
                !ExamPlanStatusEnum.IN_FORCE.getValue().equals(plan.getStatus()),
                plan.getExamPlanName() + " 未生效");

        ValidationUtils.throwIf(
                !PlanFinalConfirmedStatus.DIRECTOR_PENDING.getValue().equals(plan.getIsFinalConfirmed()),
                plan.getExamPlanName() + " 未处于待确认状态");

        return plan;
    }

    private ProjectDO getProject(ExamPlanDO plan) {
        ProjectDO project = projectMapper.selectById(plan.getExamProjectId());
        ValidationUtils.throwIfNull(project, "项目不存在");
        return project;
    }

    private List<Long> getClassroomIds(ExamPlanDO plan, Boolean isTheory) {

        List<Long> ids = isTheory
                ? examPlanMapper.getPlanExamOperClassroom(plan.getId())
                : examPlanMapper.getPlanExamTheoryClassroom(plan.getId());

        ValidationUtils.throwIfEmpty(ids, plan.getExamPlanName() + " 未分配考场");
        return ids;
    }

    private List<EnrollDO> getEnrollList(Long planId, boolean isConfirmed, String name) {
        List<EnrollDO> list = enrollMapper.selectList(
                new LambdaQueryWrapper<EnrollDO>()
                        .eq(EnrollDO::getExamPlanId, planId)
                        .orderByAsc(EnrollDO::getId)
        );

        ValidationUtils.throwIf(ObjectUtil.isEmpty(list) && isConfirmed, name + " 无报名数据");
        return list;
    }

    private void savePlanApplyClass(Long planId, List<EnrollDO> enrollList) {
        List<PlanApplyClassDO> list = enrollList.stream()
                .map(EnrollDO::getClassId)
                .distinct()
                .map(classId -> {
                    PlanApplyClassDO d = new PlanApplyClassDO();
                    d.setPlanId(planId);
        ...
        if (!list.isEmpty()) {
            planApplyClassMapper.insertBatch(list);
        }
    }

    private List<InvigilatorAssignResp> getInvigilators(Long planId, String name) {
        List<InvigilatorAssignResp> list = planInvigilateMapper.getListByPlanId(planId);
        ValidationUtils.throwIfEmpty(list, name + " 未配置监考员");
        return list;
    }

    private boolean isRejected(Integer status) {
        return PlanFinalConfirmedStatus.DIRECTOR_REJECTED.getValue().equals(status);
    }

    private boolean handleReject(Long planId, Integer status) {

        LambdaUpdateWrapper<ExamPlanDO> wrapper = new LambdaUpdateWrapper<>();
        wrapper.eq(ExamPlanDO::getId, planId)
                .set(ExamPlanDO::getIsFinalConfirmed, status)
                .set(ExamPlanDO::getAssignType, null);

        planInvigilateMapper.delete(
                new LambdaQueryWrapper<PlanInvigilateDO>()
                        .eq(PlanInvigilateDO::getExamPlanId, planId));

        planClassroomMapper.delete(
    ...
    private boolean updatePlanStatus(Long planId, Integer status, Boolean isTheory) {
        return baseMapper.update(null,
                new LambdaUpdateWrapper<ExamPlanDO>()
                        .eq(ExamPlanDO::getId, planId)
                        .set(ExamPlanDO::getIsFinalConfirmed, status)
                        .set(isTheory, ExamPlanDO::getStatus, PlanConstant.OVER.getStatus())
        ) > 0;
    }


    private void generateExamPassword(List<InvigilatorAssignResp> list) {

        Map<Long, List<InvigilatorAssignResp>> group =
                list.stream().collect(Collectors.groupingBy(InvigilatorAssignResp::getClassroomId));

        List<PlanInvigilateDO> updateList = new ArrayList<>();

        for (Map.Entry<Long, List<InvigilatorAssignResp>> entry : group.entrySet()) {
            String pwd = generateExamPassword();

            for (InvigilatorAssignResp item : entry.getValue()) {
                PlanInvigilateDO d = new PlanInvigilateDO();
                d.setId(item.getId());
                d.setExamPassword(pwd);
                updateList.add(d);
            }
        }
    ...
    private void handleExamArrangement(
            ExamPlanDO plan,
            ProjectDO project,
    ...
    private void generateExamNumber(
        ...
        for (EnrollDO enroll : enrollList) {
            ...
            String examNumber = projectCode + planCount + examDate +
    ...
    private void saveTicketsAndPapers(
            ExamPlanDO plan,
            List<EnrollDO> enrollList,
        ...
        for (EnrollDO enroll : enrollList) {
            ...
            // 准考证
            String url = safeGenerateWorkerTicket(
                    user.getId(),
                    user.getUsername(),
                    user.getNickname(),
                    enroll.getExamNumber(),
                    enroll.getClassId(),
                    classMap.get(enroll.getClassId())
            );
    ...
    public void exemptTheory(Long planId, List<EnrollDO> enrollList) {
        ...
        List<ExamRecordsDO> examRecordsList = enrollList.stream().map(item -> {
            ...
            examRecordsDO.setRoadInputStatus(hasRoad
                    ...
                    : ExamScoreEntryStatusEnum.ENTERED.getValue());
            examRecordsDO.setAttemptType(ExamRecordAttemptEnum.FIRST.getValue());
            examRecordsDO.setExamResultStatus((hasOper || hasRoad) ? ExamResultStatusEnum.NOT_ENTERED.getValue() : ExamResultStatusEnum.PASSED.getValue());
            return examRecordsDO;
        }).toList();
    ...
    private Map<Long, String> getClassMap(List<EnrollDO> list) {
        List<Long> classIds = list.stream().map(EnrollDO::getClassId).distinct().toList();
        return orgClassMapper.selectByIds(classIds)
                .stream()
                .collect(Collectors.toMap(
                        OrgClassDO::getId,
                        c -> Optional.ofNullable(c.getClassName()).orElse("")
                ));
    }

    private boolean isTheory(ProjectDO project) {
        return !HasExamEnum.NO.getValue().equals(project.getIsTheory());
    }

    private boolean isWorker(ExamPlanDO plan) {
...
//    }
    ...
    public Boolean inspectorCenterDirectorConform(Long planId, Integer isFinalConfirmed) {
        ...
        // 主任确认 && 检验检测人员考试 -> 只生成试卷
        if (success && isConfirmed && ExamPlanTypeEnum.INSPECTION.getValue()
                .equals(examPlanDO.getPlanType()) && projectDO.getIsTheory() != null && projectDO
                .getIsTheory() == 1 && projectDO.getProjectLevel() != null && projectDO.getProjectLevel() == 1) {
