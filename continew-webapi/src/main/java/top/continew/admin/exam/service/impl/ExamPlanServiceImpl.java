...
public class ExamPlanServiceImpl extends BaseServiceImpl<ExamPlanMapper, ExamPlanDO, ExamPlanResp, ExamPlanDetailResp, ExamPlanQuery, ExamPlanReq> implements ExamPlanService {
    ...
    private ExamPlanDO getAndValidatePlan(Long planId) {
        ...
        ValidationUtils.throwIf(!ExamPlanStatusEnum.IN_FORCE.getValue().equals(plan.getStatus()), plan
            .getExamPlanName() + " 未生效");

        ValidationUtils.throwIf(!PlanFinalConfirmedStatus.DIRECTOR_PENDING.getValue()
            .equals(plan.getIsFinalConfirmed()), plan.getExamPlanName() + " 未处于待确认状态");
    ...
    private List<EnrollDO> getEnrollList(Long planId, boolean isConfirmed, String name) {
        List<EnrollDO> list = enrollMapper.selectList(new LambdaQueryWrapper<EnrollDO>()
    ...
    private void savePlanApplyClass(Long planId, List<EnrollDO> enrollList) {
        List<PlanApplyClassDO> list = enrollList.stream().map(EnrollDO::getClassId).distinct().map(classId -> {
    ...
    private boolean updatePlanStatus(Long planId, Integer status, Boolean isTheory) {
        return baseMapper.update(null, new LambdaUpdateWrapper<ExamPlanDO>().eq(ExamPlanDO::getId, planId)
            .set(ExamPlanDO::getIsFinalConfirmed, status)
            .set(isTheory, ExamPlanDO::getStatus, PlanConstant.OVER.getStatus())) > 0;
    }

    private void generateExamPassword(List<InvigilatorAssignResp> list) {

        Map<Long, List<InvigilatorAssignResp>> group = list.stream()
    ...
    private void handleExamArrangement(ExamPlanDO plan,
                                       ProjectDO project,
                                       List<EnrollDO> enrollList,
                                       List<Long> classroomIds,
                                       Boolean isTheory) {
