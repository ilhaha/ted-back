...
public class WorkerApplyServiceImpl extends BaseServiceImpl<WorkerApplyMapper, WorkerApplyDO, WorkerApplyResp, WorkerApplyDetailResp, WorkerApplyQuery, WorkerApplyReq> implements WorkerApplyService {
        ...
        if (isWelding) {
            List<WeldingExamApplicationDO> weldingExamApplicationDOS = weldingExamApplicationMapper
                .selectList(new LambdaQueryWrapper<WeldingExamApplicationDO>()
                    ...
                    .eq(WeldingExamApplicationDO::getWeldingType, weldingConfig.getMetalProjectIdList()
                        .contains(projectId) ? WeldingTypeEnum.METAL.getValue() : WeldingTypeEnum.NON_METAL.getValue())
    ...
    public Boolean orgImport(List<WorkerOrgImportReq> workerOrgImportReqs) {
        ...
        if (isWelding) {
            ...
            if (weldingConfig.getMetalProjectIdList().contains(projectId)) {
            ...
            if (!notExistCodes.isEmpty()) {
                weldingExamApplicationMapper.insertBatch(notExistCodes.stream().map(code -> {
    ...
    public Boolean orgSingleUploadDoc(WorkerOrgUploadReq req) {
        ...
        update.setEducation(req.getEducation());
